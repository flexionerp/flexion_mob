    ...
    return (
        <View style={{ width: '90%' }} >
            <View style={styles.container}>
                {preReserve.length > 0 && <TouchableOpacity disabled={preReserve.length == 0 ? true : false} onPress={() => navigation.navigate(SCREENS.STATSLISTING, { label: "Pre-Reserved", list: preReserve })} style={[styles.top]}>
                ...
                {notReleased.length > 0 && booked.length > 200 && <TouchableOpacity disabled={booked.length < 200 ? true : false} onPress={() => navigation.navigate(SCREENS.DASHBOARDLISTING, { label: "Not Released", list: notReleased })} style={[styles.top]}>
                    <Image source={ICONS.building} style={{ width: 40, height: 40, marginRight: 10 }} />
                    {booked.length < 200
                        ?
                ...
                {reserve.length > 0 && <TouchableOpacity disabled={reserve.length == 0 ? true : false} onPress={() => navigation.navigate(SCREENS.STATSLISTING, { label: "Reserved", list: reserve })} style={[styles.top]}>
                    <Image source={ICONS.building} style={{ width: 40, height: 40, marginRight: 10 }} />
                ...
                {homeList.length > 0 && <TouchableOpacity disabled={homeList.length == 0 ? true : false} onPress={() => navigation.navigate("Home")} style={styles.top}>
                    <Image source={ICONS.building} style={{ width: 40, height: 40, marginRight: 10 }} />
                    <Text style={styles.rightCount}>{homeList.length}</Text>
                    <Text style={styles.rightLabel}>TOTAL Approval In-Progress</Text>
                </TouchableOpacity>
                }
                {booked.length > 0 && <TouchableOpacity onPress={() => navigation.navigate(SCREENS.STATSLISTING, { label: "Booked", list: booked })} style={[styles.top]}>
                    <Image source={ICONS.building} style={{ width: 40, height: 40, marginRight: 10 }} />
                    <Text style={styles.rightCount}>{booked.length}</Text>
                    <Text style={styles.rightLabel}>TOTAL Booked</Text>
                </TouchableOpacity>
                }
                {booked.length > 0 && !isCustomer && <TouchableOpacity disabled={isCustomer} onPress={() => navigation.navigate(SCREENS.STATSLISTING, { label: "Cancellation", list: cancelllation })} style={[styles.top]}>
                    <Image source={ICONS.building} style={{ width: 40, height: 40, marginRight: 10 }} />
                    <>
                        {
                            isCustomer
                    ...
                    <Text style={styles.rightLabel}>TOTAL Cancellation In-Progress</Text>
                </TouchableOpacity>
                }
            </View>
            <View style={{ width: '100%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap:'wrap' }}>
                <BtnLI
                    lable={"Unit Details"}
                    size={"48%"}
                    onClick={() => navigation.navigate("Unit")}
                    icon={ICONS.unit}
                />
                <BtnLI
                    lable={"SOA"}
                ...
                <BtnLI
                    lable={"Pay now"}
                    size={"48%"}
                    onClick={() => navigation.navigate('PayNowDashboard')}
                    icon={ICONS.card}
                />
            </View>
           
            {
                !isCustomer
                        ...
                        booked.length > 200 && <View style={styles.container2nd}>
                            ...
                            <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalSales", { data: saleSummeryStats })} style={styles.card} >
                                <View style={styles.bottomGrediant} >
                                    <Text style={[styles.rightCount, { width: '50%' }]}>{saleSummery && Math.abs(saleSummery).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                                    <Text style={styles.labelStyle}>TOTAL Sale</Text>
                                </View>
                            </TouchableOpacity>
                            <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalSales", { data: saleSummeryStats })} style={styles.card} >
                                <View style={styles.bottomGrediant} >
                                    <Text style={[styles.rightCount, { width: '50%' }]}>{Math.abs(dues).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                            ...
                            <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalSales", { data: saleSummeryTowerAStats })} style={styles.card} >
                                <View style={styles.bottomGrediant} >
                                    <Text style={[styles.rightCount, { width: '50%' }]}>{saleSummeryA && Math.abs(saleSummeryA).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                            ...
                            <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalSales", { data: saleSummeryTowerBStats })} style={styles.card} >
                                <View style={styles.bottomGrediant} >
                                    <Text style={[styles.rightCount, { width: '50%' }]}>{saleSummeryB && Math.abs(saleSummeryB).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                            ...
                            <TouchableOpacity disabled={true} onPress={() => navigation.navigate("SaleCancellation", { data: cancelDataStats })} style={styles.card} >
                                <View style={styles.bottomGrediant} >
                                    <Text style={[styles.rightCount, { width: '50%' }]}>{cancellation && Math.abs(cancellation).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                            ...
                            <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalCollections", { data: collections })} style={styles.card} >
                                <View style={styles.bottomGrediant} >
                                    <Text style={[styles.rightCount, { width: '50%' }]}>{collectionsCount && Math.abs(collectionsCount).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                    ...
                    <View style={styles.container2nd}>
                        ...
                        <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalSales", { data: saleSummeryStats })} style={styles.card} >
                            <View style={styles.bottomGrediant} >
                                <Text style={[styles.rightCount, { width: '50%' }]}>{customerReceivableList[0] && Math.abs(customerReceivableList[0].RESERVE_SALES).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                                <Text style={styles.labelStyle}>Book Sales</Text>
                            </View>
                        </TouchableOpacity>
                        <TouchableOpacity disabled={true} onPress={() => navigation.navigate("SaleCancellation", { data: cancelDataStats })} style={styles.card} >
                            <View style={styles.bottomGrediant}>
                                <Text style={[styles.rightCount, { width: '50%' }]}>{customerReceivableList[0] && Math.abs(customerReceivableList[0].INVOICED_AMOUNT).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                        ...
                        <TouchableOpacity disabled={true} onPress={() => navigation.navigate("SaleCancellation", { data: cancelDataStats })} style={styles.card} >
                            <View style={styles.bottomGrediant} >
                                <Text style={[styles.rightCount, { width: '50%' }]}>{customerReceivableList[0] && Math.abs(customerReceivableList[0].COLLECTED_AMOUNT).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                        ...
                        <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalCollections", { data: collections })} style={styles.card} >
                            <View style={styles.bottomGrediant} >
                                <Text style={[styles.rightCount, { width: '50%' }]}>{customerReceivableList[0] && Math.abs(customerReceivableList[0].OUTSTANDING_AMOUNT).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                                <Text style={styles.labelStyle}>Outstanding Amount</Text>
                            </View>
                        </TouchableOpacity>
                        <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalCollections", { data: collections })} style={styles.card} >
                            <View style={styles.bottomGrediant}>
                                <Text style={[styles.rightCount, { width: '50%' }]}>{customerReceivableList[0] && Math.abs(customerReceivableList[0].UNDEPOSITED_CHQ).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                        ...
                        <TouchableOpacity disabled={true} onPress={() => navigation.navigate("TotalCollections", { data: collections })} style={styles.card} >
                            <View style={styles.bottomGrediant} >
                                <Text style={[styles.rightCount, { width: '50%' }]}>{customerReceivableList[0] && Math.abs(customerReceivableList[0].NETOUTSTANDING).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
