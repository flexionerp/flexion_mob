...
export const ReceiptDetail = ({ navigation, route }) => {
    ...
    const filteredUnites = unit_list
      ? unit_list.filter((filterUnit) => {
          return (
            ...
            filterUnit.MOD_OF_PAY.toLowerCase().includes(item.toLowerCase()) ||
            moment(filterUnit.CHEQUE_DATE).format("MM-DD-YYYY").toLowerCase().includes(item.toLowerCase()) ||
            filterUnit.BANK.toLowerCase().includes(item.toLowerCase()) ||
            filterUnit.BRANCH.toLowerCase().includes(item.toLowerCase()) ||
            filterUnit.AMOUNT.toString().includes(item) ||
            filterUnit.NARRATION.toLowerCase().includes(item.toLowerCase()) ||
            filterUnit.CATEGORY.toLowerCase().includes(item.toLowerCase()) ||
            (filterUnit.CHEQUE_NO && filterUnit.CHEQUE_NO.toLowerCase().includes(item.toLowerCase())) ||
            filterUnit.IS_CHEQUE_CANCELLED.toLowerCase().includes(item.toLowerCase())
          );
        })
      : [];
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.heading}>{generalData && generalData[0] && generalData[0][0] && generalData[0][0].UNIT_CODE}</Text>
      <BackButton navigation={navigation} label="Receipt Detail" />
      {loader ? (
        <ActivityIndicator />
      ) : (
        <FlatList
