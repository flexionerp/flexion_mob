...
export const Item = ({ navigation, data }) => {
    ...
    const viewSOA = () => {
        let soaUrl = `http://tvh.flexion.ae:9092/api/Reports/customer/FPDL_INV_REG/${INVOICE_NO}/true/33`;
        navigation.navigate("StatementOfAccount", { soaUrl: soaUrl, label: "Other Charges" })
    }
            ...
            <ImageBackground
                ...
                <View style={styles.left}>
                    <View style={styles.top}>
                        <Text style={styles.unitCode}>{CATEGORY_NAME}</Text>
                        <Text style={styles.name}>{AMOUNT && Math.abs(AMOUNT).toFixed(1).replace(/\d(?=(\d{3})+\.)/g, '$&,')}</Text>
                    </View>
                    <View style={styles.center}>
                        <Text style={styles.area}>Calc Type - {CALC_TYPE}</Text>
                        <Text style={styles.area}>Due Date - {moment(DUE_DATE).format('DD-MMM-YYYY')}</Text>
                    </View>
                </View>
