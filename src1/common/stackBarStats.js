...
import { getWeeklyTowerAStats } from '../redux/property/property.action';
...
export const StackBarStats = ({ revenue }) => {
    const [label, setLabel] = useState([]);
    const [data, setData] = useState([]);
    const [colors, setColors] = useState([]);
    const dispatch = useDispatch();
    const weeklyTowerAStats = useSelector(state => state.property.weeklyTowerAStats)

    useEffect(() => {
        dispatch(getWeeklyTowerAStats())
        return () => {
        }
    }, [])
    ...
    useEffect(() => {
        ...
        let tempColors = [];
        let i = 0
        weeklyTowerAStats.forEach((element, index) => {
        ...
        <View style={{ marginTop: 16 }} >
            <View style={styles.card}>
                <View style={styles.labelRow}>
                    <Text style={styles.request} >WEEKLY TOWER A</Text>
