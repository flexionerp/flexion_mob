...
import { getSaleTowerAStats } from '../redux/property/property.action';
...
export const StackBarSaleAStats = ({ revenue }) => {
    const [label, setLabel] = useState([]);
    const [data, setData] = useState([]);
    const [colors, setColors] = useState([]);
    const dispatch = useDispatch();
    const saleSummeryTowerAStats = useSelector(state => state.property.saleSummeryTowerAStats)
    ...
    useEffect(() => {
        ...
        let tempColors = [];
        let i = 0
        saleSummeryTowerAStats.forEach((element, index) => {
        ...
        <View style={{ marginTop: 16 }} >
            <View style={styles.card}>
                <View style={styles.labelRow}>
                    <Text style={styles.request} >SALE SUMMERY TOWER A</Text>
